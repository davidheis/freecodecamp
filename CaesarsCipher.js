...
// the letters are shifted by 13 places. Thus 'A' ↔ 'N', 'B' ↔ 'O' and 
// so on.
...
function rot13(str) { // LBH QVQ VG!
  ...
  for(var i = 0; i<str.length;i++){
    if(str[i].match(/[a-z]/i)){
      newStr += (String.fromCharCode(parseInt(str[i].charCodeAt()) < 78  ? parseInt(str[i].charCodeAt()) + 13: parseInt(str[i].charCodeAt())-13));
    }else{
      newStr+= str[i];
    }  
  } 
